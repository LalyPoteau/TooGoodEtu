...
add_page('login');
add_action('login', ["email" => POST, "mdp" => POST]);

add_page('register');
add_action('register', ["email" => POST,
                        "mdp" => POST,
                        "mdp1" => POST,
                        "login" => POST]);
add_action('logout');

add_page('panierdujour');
add_page('visupanier', ["index" => GET]);
add_page('resaPanier', ["index" => GET]);
//add_action('reserver', [] => POST)
?>
